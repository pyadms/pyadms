...
#
#<admst:template match="e__dependency">
#  <admst:choose>
#    <admst:when test="[datatypename='expression']">
#      <admst:variable name="globalexpression" path="."/>
#      <admst:apply-templates select="tree" match="e__dependency"/>
#      <admst:variable name="globalexpression"/>
...
#      <admst:apply-templates select="variable" match="e__dependency"/>
...
#      <admst:apply-templates select="arg1" match="e__dependency"/>
...
#      <admst:apply-templates select="arg1|arg2" match="e__dependency"/>
#      <!--
#        +:             -:            *:            /:                                                          
...
#      <admst:apply-templates select="arg1|arg2|arg3" match="e__dependency"/>
#      <!--
#          ?: - arg1=c -  - arg1!=c -                                                            
...
#          <admst:apply-templates select="arguments[1]" match="e__dependency"/>
...
#        </admst:when>
#        <admst:otherwise>
#          <admst:apply-templates select="arguments" match="e__dependency"/>
#          <admst:choose>
...
def dependency(context, obj):
  datatypename = obj.datatypename
#  <admst:choose>
  if datatypename == "callfunction":
    f = obj.get_child('function')
    for a in f.get_children('arguments'):
      e__dependency(context, a)
    obj.set_parameter('dependency', f.get_parameter('dependency'))
  elif datatypename == "whileloop":
    #  <!--
    ...
    #       !c wb                wb                                                                                             
    #  -->
    w = obj.get_child('while')
    e__dependency(context, w)
    if context['globalopdependent'] or w.has_parameter_value('dependency', 'constant'):
      wb = obj.get_child('whileblock')
      dependency(context, wb)
    if (not context['globalopdependent']) and (w.has_parameter_value('dependency', 'constant')):
      e__dependency(context, w)
#        <admst:apply-templates select="while[dependency='constant']" match="e__dependency"/>
...
#      <admst:apply-templates select="condition" match="e__dependency"/>
...
#        <admst:apply-templates select="condition[dependency='constant']" match="e__dependency"/>
...
#      <admst:apply-templates select="case" match="e__dependency"/>
#      <admst:variable name="globaltreenode"/>
#      <admst:for-each select="caseitem">
#        <admst:for-each select="condition">
#          <admst:variable name="globaltreenode" path="."/>
#          <admst:apply-templates select="." match="e__dependency"/>
#          <admst:variable name="globaltreenode"/>
#        </admst:for-each>
...
#      <admst:apply-templates select="if" match="e__dependency"/>
#      <admst:choose>
...
#      <admst:apply-templates select="rhs" match="e__dependency"/>
#      <admst:variable name="globalcontribution"/>
...
#</admst:template>
#
#<admst:template match="adms__implicit__xml__module">
def adms__implicit__xml__module(module):
  context = {'module' : module}
  nodes = module.get_children('node')
  for n in nodes:
    n.set_parameter('grounded', n.get_parameter('location') == 'ground')

  branches = module.get_children('branch')
  for branch in branches:
    pnode = branch.get_child('pnode')
    nnode = branch.get_child('nnode')
    branch.add_child('discipline', pnode.get_child('discipline'))
    branch.set_parameter('grounded', nnode.get_parameter('grounded'))
    if not nnode.get_parameter('grounded'):
      if pnode.get_child('discipline').idx != nnode.get_child('discipline').idx:
        raise RuntimeError("disciplines on nodes of branch mismatch %d" % branch.idx)

  # is this instantiator stuff even used?
...
#  <admst:apply-templates select="(analogfunction/tree)|(analog/code)" match="dependency"/>
  for analogfunction in module.get_children('analogfunction'):
    tree = analogfunction.get_child('tree')
  for analog in module.get_children('analog'):
    code = analog.get_child('code')

  #variable = module.get_children('variable')
  #for v
...
#    </admst:for-each>
#    <admst:apply-templates select="default" match="e__dependency"/>
#    <admst:value-to
...
#</admst:template>
#
#<admst:template match="adms__implicit__xml__nature">
...
#</admst:template>
#
#<admst:template match="adms__implicit__xml">
#  <admst:apply-templates select="." match="adms__implicit__xml__nature"/>
#  <admst:apply-templates select="/module" match="adms__implicit__xml__module"/>
#</admst:template>
#

def adms__implicit__xml(admsmain):
  for module in admsmain.get_children('module'):
    adms__implicit__xml__module(module)
